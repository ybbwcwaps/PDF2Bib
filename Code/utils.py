...
class Pdf2Ref:
    ...
    def GetRefPages(self):
        ...
        ref_list = [i.replace('\n', ' ') for i in ref_list]
        references = ' '.join(ref_list).replace(
            '- ', '-')  # This is followed by a word wrap
        self.pdf.close()
    ...
    def GetUnitRef(self):
        ...
        ref_list = list(filter(None, ref_list))   # Filter empty string
    ...
    def GetTitle(self):
        ...
        for f in self.ref_list:
            ...
            # Several cases: beginning and ending Spaces, middle line breaks, and ending numbers
            self.title_list.append(re.sub(
                r'[\d\s]*$', '', (re.sub(r'\s+', ' ', sentences[2][:].replace('\n', ' ').lstrip().rstrip())) + "  "))
